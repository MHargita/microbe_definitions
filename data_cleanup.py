...
#Create 'Number of Days' column that equals the difference between Specimen Collection Day and Admit Day
#transformed_data_df[['Admit Day','Date']] = transformed_data_df[['Admit Day','Date']].apply(pd.to_datetime) #if conversion required
#transformed_data_df['Number of Days'] = (transformed_data_df['Date'] - transformed_data_df['Admit Day']).dt.days

#define HAI condition
#def HAI(s):
    #if (s['Number of Days'] >= 3):
        #return '1'
  #  else:
        #return '0'
#transformed_data_df['HAI'] = transformed_data_df.apply(HAI, axis=1)
#del transformed_data_df['Number of Days']

#Define keep or toss by writing an if statement with count of 'common organism' having to be 5 or less within a certain date range (a week)
#If uncommon organism the count definition does not apply
